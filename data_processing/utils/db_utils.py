...
import sqlite3
import pandas as pd
import streamlit as st  # To display messages in the Streamlit app
...
def load_data(db_path="flights_database.db"):
    try:
        ...
        if "flights" not in table_names:
            st.error(f"Error: The 'flights' table does not exist in the database '{db_path}'.")
        ...
        st.error("Database error occurred. Please ensure the database is valid and the required tables exist.")
        st.error(f"Original error message: {e}")
        raise
